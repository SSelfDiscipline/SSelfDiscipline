...
# 自定义函数
def Img2label_paths(img_path,i):
    yololabel_path = img_path.replace("split", "yololabel").strip(".txt")

    imgi_label_path = yololabel_path + os.sep + str(i) + ".txt"

    return imgi_label_path
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        try:
            ...
            # 因为是自己生成的数据集，这步其实没必要
            self.im_files = [x for x in f if x.split(".")[-1].lower() in IMG_FORMATS]
        ...
        self.label_files = [Img2label_paths(path,i) for i in range(len(self.im_files))]
        ...
        assert nf == len(self.im_files),'数据发送错误，请修改！'
        # 图片大小
        ...
        # Update labels，可以过滤成几类或者只识别是正样本还是负样本
        ...
        #     if single_cls:  # single-class training, merge all classes into 0,每个样本只有一张图片
        #         self.labels[i][:, 0] = 0
        ...
        #         if maxi < 1:
        #             # 如果全部都是宽大于高比例小于1，那么直接按照下面这种方式进行训练，减少无关的黑边，减少计算量
        #             shapes[i] = [maxi, 1]
        #         elif mini > 1:
        #             shapes[i] = [1, 1 / mini]
